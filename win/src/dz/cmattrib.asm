...
StringToTime proc private lpTime:ptr char_t, lpDate:ptr char_t
  ...
  local fblk:   ptr S_FBLK,
    ...
    .if rsopen(IDD_DZFileAttributes)
        ...
        .if wsfindfirst(fname, edi, 00FFh) != -1
            ...
            FileTimeToStringA([ebx].S_TOBJ.to_data[ID_ACCESSTIME], &[edi].WIN32_FIND_DATA.ftLastAccessTime)
            FileDateToStringA([ebx].S_TOBJ.to_data[ID_ACCESSDATE], &[edi].WIN32_FIND_DATA.ftLastAccessTime)
            FileTimeToStringA([ebx].S_TOBJ.to_data[ID_CREATETIME], &[edi].WIN32_FIND_DATA.ftCreationTime)
            FileDateToStringA([ebx].S_TOBJ.to_data[ID_CREATEDATE], &[edi].WIN32_FIND_DATA.ftCreationTime)
            FileTimeToStringA([ebx].S_TOBJ.to_data[ID_MODTIME], &[edi].WIN32_FIND_DATA.ftLastWriteTime)
            FileDateToStringA([ebx].S_TOBJ.to_data[ID_MODDATE], &[edi].WIN32_FIND_DATA.ftLastWriteTime)
            ...
            .if dlevent(ebx)
                ...
                .if osopen(fname, _A_NORMAL, M_WRONLY, A_OPEN) != -1
                    mov esi,eax
                    setftime(esi,
                        StringToTime([ebx].S_TOBJ.to_data[ID_MODTIME], [ebx].S_TOBJ.to_data[ID_MODDATE]))
                    setftime_create(esi,
                        StringToTime([ebx].S_TOBJ.to_data[ID_CREATETIME], [ebx].S_TOBJ.to_data[ID_CREATEDATE]))
                    setftime_access(esi,
                        StringToTime([ebx].S_TOBJ.to_data[ID_ACCESSTIME], [ebx].S_TOBJ.to_data[ID_ACCESSDATE]))
                    _close(esi)
                .endif
            .endif
        .endif
